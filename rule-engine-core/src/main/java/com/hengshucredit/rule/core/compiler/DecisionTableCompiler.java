...
public class DecisionTableCompiler implements RuleCompiler {
    ...
    private CompileResult doCompile(String modelJson, VarContext varContext) {
        try {
            ...
            LinkedHashSet<String> outputVarCodes = collectOutputVarCodes(rules, globalActionDefs, varContext);
            ...
            for (int i = 0; i < rules.size(); i++) {
                ...
                script.append(buildRulePredicate(rule, ruleConditions, legacyColumnDefs, varContext));
                script.append(") {\n");

                if (isUnique) {
                    script.append("    _uniqueHitCount = _uniqueHitCount + 1;\n");
                }
                appendRuleAssignments(script, ruleActions, globalActionDefs, varContext);
