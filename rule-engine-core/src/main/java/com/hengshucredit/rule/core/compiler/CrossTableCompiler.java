...
public class CrossTableCompiler implements RuleCompiler {
    ...
    private CompileResult doCompile(String modelJson, VarContext varContext) {
        try {
            ...
            if (model.getJSONObject("rowVar") != null && model.getJSONObject("colVar") != null) {
                return compileSimpleMatrix(model, varContext);
            }
            ...
            if (rowDefs != null) {
                for (int i = 0; i < rowDefs.size(); i++) {
                    ...
                    if (outVar != null && !outVar.isEmpty()) {
                        String resolved = resolveVar(varId, refType, outVar, varContext);
                    ...
                    if (conditions != null && !conditions.isEmpty()) {
                        script.append(compileRowCondition(varId, refType, varCode, varType, conditions, varContext));
                    }
                }
            }
            ...
            if (cells != null && !cells.isEmpty()) {
                for (int i = 0; i < cells.size(); i++) {
                    ...
                    if (hasRow) {
                        ...
                        script.append(compileCellVarPredicate(rd, true, varContext));
                    }
                    if (hasRow && hasCol) {
                        script.append(" && ");
                    }
                    if (hasCol) {
                        ...
                        script.append(compileCellVarPredicate(cd, false, varContext));
                    }
                    if (!hasRow && !hasCol) {
                        script.append("true");
                    }
                    ...
                    if (action != null) {
                        ...
                        if (varCode != null && value != null) {
                            ...
                            String resolved = resolveVar(varId, refType, varCode, varContext);
    ...
    private CompileResult compileSimpleMatrix(JSONObject model, VarContext varContext) {
