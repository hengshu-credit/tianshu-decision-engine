<template>
  <div class="uiue-list-page list-library-page">
    ...
    <div class="uiue-search-container">
      <el-form :inline="true" size="small" @keyup.enter.native="handleQuery">
        ...
        <el-form-item label="名单类型">
          <el-select v-model="query.listType" clearable placeholder="全部" style="width:120px;">
    ...
    <div class="uiue-btn-bar">
      <div class="btn-right">
        <el-button type="primary" size="small" icon="el-icon-plus" @click="handleCreate">新建名单库</el-button>
      </div>
    </div>
    ...
    <el-table :data="tableData" border size="small" v-loading="loading" style="width:100%;">
      ...
      <el-table-column prop="listCode" label="名单编码" min-width="150" show-overflow-tooltip />
      <el-table-column prop="listName" label="名单名称" min-width="160" show-overflow-tooltip />
      <el-table-column label="名单类型" width="100" align="center">
        <template slot-scope="{ row }">
          <el-tag size="mini" :type="listTypeTag(row.listType)">{{ listTypeLabel(row.listType) }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="recordCount" label="有效记录" width="90" align="right" />
      ...
      <el-table-column label="操作" width="170" align="center">
        <template slot-scope="{ row }">
          <el-button type="text" size="small" @click="$router.push('/list/' + row.id)">详情</el-button>
        ...
        <el-row :gutter="12">
          ...
          <el-col :span="12">
            <el-form-item v-if="form.scope === 'PROJECT'" label="所属项目">
              <el-select v-model="form.projectId" filterable clearable placeholder="请选择项目" style="width:100%;">
        ...
        <el-row :gutter="12">
          <el-col :span="12">
            <el-form-item label="名单编码" prop="listCode">
              <el-input v-model="form.listCode" :disabled="!!form.id" placeholder="如 mobile_black_list" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        listCode: [{ required: true, message: '请输入名单编码', trigger: 'blur' }],
        listName: [{ required: true, message: '请输入名单名称', trigger: 'blur' }]
      }
    }
  },
  created() {
  ...
  methods: {
    emptyForm() {
      return { id: null, projectId: '', scope: 'PROJECT', listCode: '', listName: '', listType: 'BLACK', description: '', status: 1 }
    },
    async loadProjects() {
      try {
    ...
    handleDelete(row) {
      this.$confirm(`确定删除名单库「${row.listName}」？`, '确认删除', { type: 'warning' })
        .then(async () => {
    ...
    listTypeTag(type) {
      return { BLACK: 'danger', GREY: 'info', WHITE: 'success', OTHER: 'warning' }[type] || ''
    }
  }
}
</script>
...
<style scoped>
.module-hint { margin-bottom:12px; padding:10px 12px; background:#f5f7fa; border-radius:4px; color:#606266; }
.hint-title { font-size:14px; font-weight:600; color:#303133; margin-bottom:4px; }
