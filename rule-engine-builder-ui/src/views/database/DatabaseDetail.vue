<template>
  <div class="uiue-list-page database-detail-page">
    <div class="detail-header">
      <div>
        <div class="detail-title">{{ isCreateMode ? '新建数据库数据源' : '编辑数据库数据源' }}</div>
        <div class="detail-meta">{{ form.datasourceName || '未命名数据源' }} / {{ form.datasourceCode || '待填写编码' }}</div>
      </div>
      <div class="detail-actions">
        <el-button size="small" @click="$router.push('/database')">返回</el-button>
    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="120px" size="small" class="detail-form">
      ...
      <el-form-item label="JDBC URL" prop="jdbcUrl">
        ...
        <el-checkbox v-model="form.jdbcAutoBuild" class="jdbc-checkbox" @change="onJdbcAutoBuildChange">按上方表单自动生成 JDBC URL</el-checkbox>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      dbTypeOptions: [
        { label: 'MySQL', value: 'MYSQL', driver: 'com.mysql.cj.jdbc.Driver', validation: 'SELECT 1', port: 3306, params: 'useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&useSSL=false' },
  ...
  async created() {
    ...
    if (this.isCreateMode) {
      ...
      if (this.$route.query.projectId) this.form.projectId = Number(this.$route.query.projectId)
