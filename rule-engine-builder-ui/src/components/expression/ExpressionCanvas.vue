<template>
  <div class="expression-canvas-node">
    ...
    <div
      v-if="isSelected(path) && node && (node.kind === 'LITERAL' || node.kind === 'PATH')"
      class="canvas-inline-editor"
      @click.stop
    >
...
.canvas-inline-editor { position: relative; display: flex; min-width: 0; gap: 8px; margin: 7px 0 0 30px; padding: 8px; border: 1px solid #d7e3f2; border-radius: 7px; background: #f8fbff; }
.canvas-inline-editor .el-select { width: 108px; flex: none; }
.canvas-inline-editor .el-input { min-width: 220px; }
