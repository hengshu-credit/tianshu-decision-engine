<template>
  <el-dialog :model-value="modelValue" title="跨环境部署决策制品" width="680px" @update:model-value="$emit('update:modelValue', $event)">
    ...
    <el-form label-width="130px" class="deployment-form">
      ...
      <el-form-item label="目标规则 ID"><el-input-number v-model="form.targetDefinitionId" :min="1" /></el-form-item>
...
export default {
  ...
  data() { return { form: { environmentCode: '', targetDefinitionId: null, comment: '', bindings: {} } } },
  computed: {
    ready() {
      return Boolean(this.form.environmentCode.trim() && this.form.targetDefinitionId && this.bindingComponentIds.every(id => this.form.bindings[id]))
    }
  },
  methods: {
    submit() { this.$emit('deploy', { artifactId: this.artifactId, ...this.form, createRule: false }) }
  }
}
</script>
