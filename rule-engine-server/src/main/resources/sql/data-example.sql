...
(1, 1, 'PROJECT', 'roundTax', '金额四舍五入', '将数值保留2位小数（费用/敞口试算）', '[{"name":"amount","type":"NUMBER","label":"金额"}]', 'NUMBER', 'SCRIPT', 'return new java.math.BigDecimal(String.valueOf(amount)).setScale(2, java.math.RoundingMode.HALF_UP).doubleValue();', NULL, NULL, NULL, 1, NOW(), NOW()),
(2, 1, 'PROJECT', 'formatAmount', '金额格式化', '格式化为带千分位的字符串', '[{"name":"amount","type":"NUMBER","label":"金额"}]', 'STRING', 'SCRIPT', 'return new java.text.DecimalFormat("#,##0.00").format(amount);', NULL, NULL, NULL, 1, NOW(), NOW()),
-- JAVA 类型：通过反射实例化 Java 类，调用指定方法注册为 QL 函数
(3, 1, 'PROJECT', 'calculateVAT', '费用试算(Java)', '演示 Java 反射函数：按金额与费率计算费用', '[{"name":"amount","type":"NUMBER","label":"金额"},{"name":"rate","type":"NUMBER","label":"费率"}]', 'NUMBER', 'JAVA', NULL, 'com.hengshucredit.rule.example.functions.TaxFunctions', 'calculateVAT', NULL, 1, NOW(), NOW()),
