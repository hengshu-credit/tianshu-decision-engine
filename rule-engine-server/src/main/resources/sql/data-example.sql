...
-- 2. 创建项目变量
-- ============================================================
INSERT INTO `rule_variable` (`id`, `project_id`, `scope`, `var_code`, `var_label`, `script_name`, `var_type`, `var_source`, `default_value`, `example_value`, `description`, `sort_order`, `status`, `create_time`, `update_time`) VALUES
(1,  1, 'PROJECT', 'taxpayerType',       '客商类型',   'taxpayerType',       'STRING',  'INPUT',    NULL,     '一般纳税人',  '示例取值：一般纳税人/小规模纳税人（与设计器「行维度」一致）',               1, 1, NOW(), NOW()),
(2,  1, 'PROJECT', 'goodsCategory',      '产品总线',     'goodsCategory',      'STRING',  'INPUT',    NULL,     '货物',        '示例：货物/服务/不动产/无形资产（与设计器「列维度」一致）',          2, 1, NOW(), NOW()),
(3,  1, 'PROJECT', 'taxRate',             '风险定价费率',     'taxRate',             'NUMBER',  'COMPUTED', NULL,     '0.13',        '规则输出的定价/费率系数',                      3, 1, NOW(), NOW()),
(4,  1, 'PROJECT', 'annualRevenue',      '年营收(万元)',  'annualRevenue',      'NUMBER',  'INPUT',    NULL,     '5000',        '客户或主体年营业收入，单位万元',                 4, 1, NOW(), NOW()),
(5,  1, 'PROJECT', 'taxComplianceScore', '合规内控评分',  'taxComplianceScore', 'NUMBER',  'INPUT',    NULL,     '85',          '0-100 分',                               5, 1, NOW(), NOW()),
(6,  1, 'PROJECT', 'yearsInBusiness',    '合作/经营年限',      'yearsInBusiness',    'NUMBER',  'INPUT',    NULL,     '10',          '经营或与机构合作年限',                            6, 1, NOW(), NOW()),
(7,  1, 'PROJECT', 'hasViolation',       '是否存在严重违规', 'hasViolation',       'BOOLEAN', 'INPUT',    'false',  'false',       'true/false',                             7, 1, NOW(), NOW()),
(8,  1, 'PROJECT', 'creditLevel',        '内部信用等级',      'creditLevel',        'STRING',  'COMPUTED', NULL,     'A',           '决策树输出：A/B/C/D',                     8, 1, NOW(), NOW()),
(9,  1, 'PROJECT', 'totalAmount',        '交易金额(含税口径)',      'totalAmount',        'NUMBER',  'INPUT',    NULL,     '113000',      '用于试算或限额校验的金额示例',                             9, 1, NOW(), NOW()),
(10, 1, 'PROJECT', 'isExempt',           '是否适用减免政策',   'isExempt',           'BOOLEAN', 'INPUT',    'false',  'false',       '是否命中费用减免或优惠策略',                    10, 1, NOW(), NOW()),
(11, 1, 'PROJECT', 'taxBurdenDeviation', '指标偏离度',     'taxBurdenDeviation', 'NUMBER',  'INPUT',    NULL,     '0.15',        '与行业或客群基准的偏离比例，如 0.15 表示 15%',       11, 1, NOW(), NOW()),
(12, 1, 'PROJECT', 'violationCount',     '历史风险事件次数',   'violationCount',     'NUMBER',  'INPUT',    NULL,     '0',           '历史负面事件计数',                           12, 1, NOW(), NOW()),
(13, 1, 'PROJECT', 'totalScore',         '风险总评分',     'totalScore',         'NUMBER',  'COMPUTED', NULL,     '85',          '评分卡计算输出的总分',                    13, 1, NOW(), NOW()),
(14, 1, 'PROJECT', 'riskLevel',          '风险等级',       'riskLevel',          'STRING',  'COMPUTED', NULL,     '低风险',      '评分卡输出：低风险/中风险/高风险',          14, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE `scope` = 'PROJECT', `var_label` = VALUES(`var_label`);
...
-- ============================================================
INSERT INTO `rule_function` (`id`, `project_id`, `scope`, `func_code`, `func_name`, `description`, `params_json`, `return_type`, `impl_type`, `impl_script`, `impl_class`, `impl_method`, `impl_bean_name`, `status`, `create_time`, `update_time`) VALUES
-- SCRIPT 类型：QLExpress 脚本实现，执行时自动包装为 function 定义拼接在编译脚本前面
(1, 1, 'PROJECT', 'roundTax', '金额四舍五入', '将数值保留2位小数（费用/敞口试算）', '[{"name":"amount","type":"NUMBER","label":"金额"}]', 'NUMBER', 'SCRIPT', 'scaled = amount * 100 + 0.5; return (scaled - scaled % 1) / 100;', NULL, NULL, NULL, 1, NOW(), NOW()),
(2, 1, 'PROJECT', 'formatAmount', '金额格式化', '格式化为带千分位的字符串', '[{"name":"amount","type":"NUMBER","label":"金额"}]', 'STRING', 'JAVA', NULL, 'com.hengshucredit.rule.server.functions.TaxFunctions', 'formatAmount', NULL, 1, NOW(), NOW()),
-- JAVA 类型：通过反射实例化 Java 类，调用指定方法注册为 QL 函数
(3, 1, 'PROJECT', 'calculateVAT', '费用试算(Java)', '演示 Java 反射函数：按金额与费率计算费用', '[{"name":"amount","type":"NUMBER","label":"金额"},{"name":"rate","type":"NUMBER","label":"费率"}]', 'NUMBER', 'JAVA', NULL, 'com.hengshucredit.rule.server.functions.TaxFunctions', 'calculateVAT', NULL, 1, NOW(), NOW()),
-- BEAN 类型：从 Spring 容器获取 Bean，调用指定方法注册为 QL 函数
(4, 1, 'PROJECT', 'calcTaxByBean', '费用试算(Bean)', '演示 Spring Bean 函数，复用已有定价服务', '[{"name":"amount","type":"NUMBER","label":"金额"},{"name":"rate","type":"NUMBER","label":"费率"}]', 'NUMBER', 'BEAN', NULL, NULL, 'calculateVAT', 'taxFunctions', 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE `scope` = 'PROJECT', `func_name` = VALUES(`func_name`), `impl_type` = VALUES(`impl_type`), `impl_script` = VALUES(`impl_script`), `impl_class` = VALUES(`impl_class`), `impl_method` = VALUES(`impl_method`), `impl_bean_name` = VALUES(`impl_bean_name`);

-- ============================================================
-- 2.2 数据对象定义（对象传入）
-- ============================================================
INSERT INTO `rule_data_object` (`id`, `project_id`, `scope`, `object_code`, `object_label`, `script_name`, `object_type`, `source_type`, `source_content`, `status`, `create_time`, `update_time`) VALUES
(1, 1, 'PROJECT', 'TaxContext', '风控上下文对象', 'taxContext', 'INPUT', 'JSON', '{"taxpayerType":"STRING","goodsCategory":"STRING","totalAmount":"NUMBER","isExempt":"BOOLEAN"}', 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE `scope` = 'PROJECT', `object_label` = VALUES(`object_label`);

-- ============================================================
-- 3. 创建八条规则定义（已发布状态，含对象传入和函数调用示例）
-- ============================================================
INSERT INTO `rule_definition` (`id`, `project_id`, `rule_code`, `rule_name`, `model_type`, `description`, `current_version`, `published_version`, `status`, `create_by`, `create_time`) VALUES
(1, 1, 'RC_PRICING_TABLE',        '客商×产品总线定价表',     'TABLE', '根据客商类型与产品总线匹配风险定价费率（决策表示例）',                              1, 1, 1, 'system', NOW()),
(2, 1, 'RC_CREDIT_TREE', '客户信用分层', 'TREE',  '结合负面记录、合规评分、营收规模、合作年限输出内部信用等级 A/B/C/D',          1, 1, 1, 'system', NOW()),
(3, 1, 'RC_EXPOSURE_FLOW',        '敞口与费用试算流程',     'FLOW',  '串行试算：定价费率 → 本金/敞口拆分 → 费用 → 减免策略 → 应收费用', 1, 1, 1, 'system', NOW()),
(4, 1, 'RC_RATE_MATRIX',     '风险定价交叉表',       'CROSS', '客商类型 × 产品总线 交叉矩阵输出风险定价费率（与交叉表设计器示例一致）',                         1, 1, 1, 'system', NOW()),
(5, 1, 'RC_RISK_SCORECARD',      '综合风险评分卡',     'SCORE', '基于信用等级、营收、年限、指标偏离、历史事件的加权评分与风险档',            1, 1, 1, 'system', NOW()),
(6, 1, 'RC_PRICING_BY_OBJECT',      '对象传入定价', 'TABLE', '演示对象上下文入参：规则读取 taxpayerType、goodsCategory 等字段', 1, 1, 1, 'system', NOW()),
(7, 1, 'RC_FLOW_JAVA_SAMPLE',  'JAVA 函数试算',     'FLOW',  '演示 JAVA 函数：反射调用 TaxFunctions.calculateVAT 完成费用试算',      1, 1, 1, 'system', NOW()),
(8, 1, 'RC_FLOW_BEAN_SAMPLE',  'Spring Bean 试算',     'FLOW',  '演示 BEAN 函数：调用 Spring 容器内 taxFunctions.calculateVAT', 1, 1, 1, 'system', NOW())
ON DUPLICATE KEY UPDATE `scope` = 'PROJECT', `rule_name` = VALUES(`rule_name`), `description` = VALUES(`description`), `status` = 1, `published_version` = 1;
...
(1, 1,
 ...
 'QLEXPRESS', 1, NOW())
ON DUPLICATE KEY UPDATE `script_mode` = 'visual', `model_json` = VALUES(`model_json`), `compiled_script` = VALUES(`compiled_script`), `compiled_type` = VALUES(`compiled_type`), `compile_status` = 1;
...
-- ============================================================
INSERT INTO `rule_variable` (`id`, `project_id`, `scope`, `var_code`, `var_label`, `script_name`, `var_type`, `var_source`, `default_value`, `example_value`, `description`, `sort_order`, `status`, `create_time`, `update_time`) VALUES
(15, 1, 'PROJECT', 'serviceType',             '业务类型',       'serviceType',             'ENUM',   'INPUT',    NULL,    '基础通信', '基础通信/增值业务/宽带接入/ICT服务',         15, 1, NOW(), NOW()),
(16, 1, 'PROJECT', 'paymentMode',             '结算方式',       'paymentMode',             'ENUM',   'INPUT',    NULL,    '后付费',   '预付费/后付费',                           16, 1, NOW(), NOW()),
(17, 1, 'PROJECT', 'customerType',            '客户类型',       'customerType',            'ENUM',   'INPUT',    NULL,    '企业客户', '企业客户/个人客户/政企客户',                17, 1, NOW(), NOW()),
(18, 1, 'PROJECT', 'taxpayerQualification',   '纳税人资格',     'taxpayerQualification',   'ENUM',   'INPUT',    NULL,    '一般纳税人', '一般纳税人/小规模纳税人',                18, 1, NOW(), NOW()),
(19, 1, 'PROJECT', 'multiDimRate',           '多维组合定价系数',   'multiDimRate',           'NUMBER', 'COMPUTED', NULL,    '0.09',     '复杂交叉表/QL脚本输出的组合定价系数',          19, 1, NOW(), NOW()),
(20, 1, 'PROJECT', 'customerLevel',           '客户等级',       'customerLevel',           'ENUM',   'INPUT',    NULL,    '金',       '钻石/金/银/铜',                          20, 1, NOW(), NOW()),
(21, 1, 'PROJECT', 'monthlyConsumption',      '月消费金额(元)', 'monthlyConsumption',      'NUMBER', 'INPUT',    NULL,    '5000',     '月均消费额（元）',                        21, 1, NOW(), NOW()),
(22, 1, 'PROJECT', 'invoiceDeviationRate',    '开票偏差率',     'invoiceDeviationRate',    'NUMBER', 'INPUT',    NULL,    '0.05',     '月开票金额与消费金额的偏差百分比',          22, 1, NOW(), NOW()),
(23, 1, 'PROJECT', 'redInvoiceRatio',         '红冲发票比例',   'redInvoiceRatio',         'NUMBER', 'INPUT',    NULL,    '0.02',     '红冲发票占总开票量的百分比',               23, 1, NOW(), NOW()),
(24, 1, 'PROJECT', 'zeroRateInvoiceRatio',    '零税率发票占比', 'zeroRateInvoiceRatio',    'NUMBER', 'INPUT',    NULL,    '0.01',     '零税率发票金额占比',                      24, 1, NOW(), NOW()),
(25, 1, 'PROJECT', 'crossRegionInvoiceRatio', '跨地区开票比例', 'crossRegionInvoiceRatio', 'NUMBER', 'INPUT',    NULL,    '0.08',     '跨地区开票金额占比',                      25, 1, NOW(), NOW()),
(26, 1, 'PROJECT', 'invoiceRiskScore',        '发票风险评分',   'invoiceRiskScore',        'NUMBER', 'COMPUTED', NULL,    '135',      '复杂评分卡输出的风险总评分',               26, 1, NOW(), NOW()),
(27, 1, 'PROJECT', 'invoiceRiskLevel',        '发票风险等级',   'invoiceRiskLevel',        'STRING', 'COMPUTED', NULL,    '低风险',   '评分卡输出：低风险/中风险/高风险',          27, 1, NOW(), NOW()),
(28, 1, 'PROJECT', 'billingAmount',           '含税账单金额(元)', 'billingAmount',         'NUMBER', 'INPUT',    NULL,    '100000',   'QL脚本输入：含税账单总金额',               28, 1, NOW(), NOW()),
(29, 1, 'PROJECT', 'basicServiceRatio',       '基础通信占比',   'basicServiceRatio',       'NUMBER', 'INPUT',    NULL,    '0.6',      '混合经营中基础通信收入占比',               29, 1, NOW(), NOW()),
(30, 1, 'PROJECT', 'vasServiceRatio',         '增值业务占比',   'vasServiceRatio',         'NUMBER', 'INPUT',    NULL,    '0.4',      '混合经营中增值业务收入占比',               30, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE `scope` = 'PROJECT', `var_label` = VALUES(`var_label`);
...
-- ============================================================
INSERT INTO `rule_definition` (`id`, `project_id`, `rule_code`, `rule_name`, `model_type`, `description`, `current_version`, `published_version`, `status`, `create_by`, `create_time`) VALUES
(9,  1, 'RC_MULTI_DIM_RATE',    '交叉矩阵多维定价（8×6）',       'CROSS_ADV', '行维度：业务类型×结算方式（8）；列维度：客户类型×纳税人资格（6）。与复杂交叉矩阵设计器示例一致（含 ICT 后付一般纳税人 0.13）', 1, 1, 1, 'system', NOW()),
(10, 1, 'RC_INVOICE_FRAUD_SCORE', '交易票据异常评分',       'SCORE_ADV', '基于客户等级、交易规模、票据偏离与地域异常等指标的分组加权评分（复杂评分卡示例）', 1, 1, 1, 'system', NOW()),
(11, 1, 'RC_BLEND_CALC_SCRIPT',    '混业组合计费脚本', 'SCRIPT',    '多费率产品线按比例分拆计费，并支持简易计税口径（QL脚本示例）', 1, 1, 1, 'system', NOW())
ON DUPLICATE KEY UPDATE `scope` = 'PROJECT', `rule_name` = VALUES(`rule_name`), `description` = VALUES(`description`), `status` = 1, `published_version` = 1;
...
(0, 'GLOBAL', 'EMPTY_LIST', '空列表', 'EMPTY_LIST', 'LIST', 'CONSTANT', NULL, '[]', '', '[]', '系统常量：空列表，可在条件右侧通过字段选择器引用。', -997, 1, NOW(), NOW()),
(0, 'GLOBAL', 'EMPTY_MAP', '空映射', 'EMPTY_MAP', 'MAP', 'CONSTANT', NULL, '{}', '', '{}', '系统常量：空键值映射，可在条件右侧通过字段选择器引用。', -996, 1, NOW(), NOW()),
...
(0, 'GLOBAL', 'POSITIVE_INFINITY', '正无穷', 'POSITIVE_INFINITY', 'DOUBLE', 'CONSTANT', NULL, 'Infinity', '', 'Infinity', '系统常量：数值比较上界，可在条件右侧通过字段选择器引用。', -990, 1, NOW(), NOW())
