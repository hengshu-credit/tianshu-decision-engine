...
> **重要提示**: 当前为未发布版，功能待增加，有部分功能逻辑还在修缮中 ......
...
后端 Java 包名前缀统一为 `com.hengshucredit`，数据库表名、Redis 频道和前端 API 路由保持业务语义不变。
...
请求头、Query、入参映射、响应映射、请求体模板、鉴权配置、兜底返回等 JSON 配置均使用 Monaco JSON 编辑器。API 调用测试弹窗同样使用 Monaco JSON 编辑器展示请求参数和调用结果。
